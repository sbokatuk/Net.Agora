...
/// <summary>
/// Cross-platform <see cref="IAgoraVoiceClient"/>. The platform halves live in
...
/// </summary>
public sealed partial class AgoraVoiceClient : IAgoraVoiceClient
{
    private readonly AgoraVoiceOptions _options;
    ...
    /// <inheritdoc />
    public event EventHandler<AgoraVoiceErrorEventArgs>? Error;
    ...
    public async Task JoinAsync(string channelId, CancellationToken cancellationToken = default)
    {
        ...
        await using var registration = linked.Token.Register(() =>
        {
            ...
            {
                pending.TrySetException(new AgoraVoiceException(
...
/// <summary>Called by the platform half when the SDK reports an error.</summary>
    private void RaiseError(string message, int errorCode)
    {
        ...
        _pending?.TrySetException(new AgoraVoiceException(message, errorCode));
        Error?.Invoke(this, new AgoraVoiceErrorEventArgs(message, errorCode));
    }
}
