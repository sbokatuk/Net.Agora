...
/// instead, at the point the mistake is made. Video has its own copy of this: the two packages
/// are separate assemblies and cannot be referenced from the same app anyway, since their
/// platform bindings carry the same native classes.
/// </remarks>
    ...
    internal static void Acquire()
    {
        if (Interlocked.CompareExchange(ref _taken, 1, 0) != 0)
        {
            throw new InvalidOperationException(
                "An AgoraVoiceClient is already live in this process. The Agora RTC engine is a " +
